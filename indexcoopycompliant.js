...
// TURN ON IF NOT RUNNING BETA TOKEN
	...
	setInterval(() => {
		 ...
		 console.log("Setting up POST interval for the other DBL API. (Currently broken.)")

		 setInterval(() => {
			 axios.post('https://discordbotlist.com/api/v1/bots/697523856973496430/stats', {
id: '697523856973496430',
Authorization: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ0IjoxLCJpZCI6IjY5NzUyMzg1Njk3MzQ5NjQzMCIsImlhdCI6MTU5MDUwNDA5OH0.UPWPdwTcxoJhGldq7Pb7Ds7PdSY9yA8xTyvXBt7Ip34',
...
console.log("Server count posted to discordbotlist.com!")
				 }, 900000);


		 console.log("Successfully booted Verifi.")
		 console.log("Listening on " + client.guilds.size + " servers. Woo-hoo!")
...
const memID = member.id
		...
		member.send("Please send a message in this DM with the characters you see below to begin verification for " + member.guild.name + " (Verification Step 1/2)")
									...
									msg.channel.awaitMessages(filter, { max: 1, time: 240000 }) //errors: ['time'] })
										.then(collected => {
												if (collected.first().content == ranChar1 + ranChar2 + ranChar3 + ranChar4 + ranChar5) {
													...
													if (verifi.get(`verifi_onestep_${member.guild.id}`) == '1') {
